...
package crypto {
	...
	public class SRAKey implements ISRAKey {		
		...
		private static var _instances:uint = 0; //number of SRAKey instance
		private var _currentInstance:uint = 0; //current instance number
		private var _encKey:Array = new Array(); //contained encryption key in BigInt array format
		private var _decKey:Array = new Array(); //contained decryption key in BigInt array format
		private var _modulus:Array = new Array(); //contained modulus in BigInt array format
		private static var _autoScrubInterval:Number = 500; //internal, in milliseconds, until key data is scrubbed
		private var _autoScrubIntervalID:uint; //interval identifier for current scrub timer
		...
		*/
		...
		/**
		 * Returns the starting and ending values of a QRNR range based on an offset from a starting value.
		 * 
		 * @param	startVal The starting value for the range. This value is  must be a decimal arbitrary length base 10 integer o
		 * r hexadecimal ("0x...") string.
		 * @param  range The required range of values to generate. For example, for a single card deck this value
		 * would be "52". This value is must be a decimal arbitrary length base 10 integer or hexadecimal ("0x...") 
		 * string.
		 * @param radix An optional radix value for the returned "start" and "end" values. Acceptable values are "16"
		 ...
		 * prime. The range will be larger than the range specified to accomodate the required number of values.
		 */
		public static function getQRNRRange(startVal:String, range:String, radix:uint = 16):Object {
			if (!BigInt.initialized) {
				BigInt.initialize();
			}
			var dataSize:int = int(startVal.length + 5);
			if (startVal.indexOf("0x") > -1) {												
				var startValStr:String = startVal.substr(startVal.indexOf("0x") + 2);						
				var startValArr:Array = BigInt.str2bigInt(startValStr, 16, dataSize);
			} else {												
				startValArr = BigInt.str2bigInt(startVal, 10, dataSize);
			}
			if (range.indexOf("0x") > -1) {												
				var rangeStr:String = range.substr(range.indexOf("0x") + 2);						
				var rangeValArr:Array = BigInt.str2bigInt(rangeStr, 16, dataSize);
			...
			rangeValArr = BigInt.mult(rangeValArr, two);
			rangeValArr = BigInt.add(rangeValArr, twenty);
			q2 = BigInt.add(startValArr, rangeValArr);
			var returnObj:Object = new Object();
			if (radix==16) {
				returnObj.start = "0x"+BigInt.bigInt2str(startValArr, radix);
				returnObj.end = "0x"+BigInt.bigInt2str(q2, radix);	
			} else {
				returnObj.start = BigInt.bigInt2str(startValArr, radix);
				returnObj.end = BigInt.bigInt2str(q2, radix);	
			}
			return (returnObj);
		}
