	...
	public class SmartContractManagerWidget extends PanelWidget implements IPanelWidget {
		...
		private function onNoAccountAlertClose(eventObj:Event):void {
			...
			if (eventObj.data.setAccountDetails) {
				try {
					var accountWidget:IWidget = this.getInstanceByClass("org.cg.widgets.EthereumAccountWidget")[0];
		...
		private function onDeployContract(eventObj:EthereumEvent):void {
			...
			descriptor = SmartContract.getValidatedDescriptor("PokerHandStartup", "ethereum", lounge.ethereum.client.networkID, "new", "contract", false);
			...
			descriptor = SmartContract.getValidatedDescriptor("PokerHandActions", "ethereum", lounge.ethereum.client.networkID, "new", "contract", false);
			contract = new SmartContract("PokerHandActions", lounge.ethereum.account, lounge.ethereum.password, descriptor);
			contractsArray.push(contract.address);			
			descriptor = SmartContract.getValidatedDescriptor("PokerHandResolutions", "ethereum", lounge.ethereum.client.networkID, "new", "contract", false);
			...
			contract.setAuthorizedGameContracts(contractsArray).invoke({from:lounge.ethereum.account, gas:150000});
