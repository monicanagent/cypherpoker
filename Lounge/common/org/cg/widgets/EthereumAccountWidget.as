package org.cg.widgets 
{
	...
	public class EthereumAccountWidget extends PanelWidget implements IPanelWidget {
		...
		private function onPickerListUpdate(eventObj:Event):void {
			...
			if ((password == "") && (this.accountPicker.dataProvider.length > 0)) {
				...
				if (this.lounge.ethereum!=null) {
					DebugView.addText ("   Setting default Ethereum account: " + account);
					DebugView.addText ("   Setting default Ethereum password: " + password);
		...
		private function onPickerListClick(eventObj:Event):void {
			DebugView.addText("onPickerListClick");
			this.hideNewAccountButton();
			this.hideDeleteAccountButton();
			this.hideUpdateAccountButton();
		}		
		...
		private function onOkClick(eventObj:Event):void {
			DebugView.addText("EthereumAccountWidget.onOkClick");
			if (this.accountEdit.visible == false) {				
				try {
					...
					if (lounge.ethereum!=null) {
						...
						DebugView.addText("   Updated current Ethereum account to: " + lounge.ethereum.account);
						DebugView.addText("   Updated current Ethereum password to: " + lounge.ethereum.password);
					}
					DebugView.addText("   Saving account.");
					...
					DebugView.addText("Restore to starting state in UI...");
					this.hideCancelButton();
					this.hideOkButton();
					this.hidePasswordEdit();
					this.hideSavePaswordCheck();
				...
				if (this.lounge.ethereum!=null) {
					...
					lounge.ethereum.password = this.passwordEdit.text;
					DebugView.addText("   Updated default Ethereum account to: " + lounge.ethereum.account);
					DebugView.addText("   Updated default Ethereum password to: " + lounge.ethereum.password);
				}
		...
		private function createNewAccount():void {
			...
			DebugView.addText("EthereumAccountWidget.createNewAccount");
